...
Each person on the system must log in using a user account which either allows the person to access specific files and directories or disallows such access, accomplished using file permissions, which are file and directory permissions given by the system to users, groups and everyone else who logs in.

User accounts also belong to groups, which can also be used to provide access to files/directories. 
