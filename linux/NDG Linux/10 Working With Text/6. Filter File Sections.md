...
+ It’s primarily used for working with delimited database files.
+ Again, delimited files are files that contain columns separated by a delimiter. 
...
+ The `-f` option can specify which fields to display, either as a hyphenated (واصلة) range or a comma-separated list.
...
## Cut fixed range:
+ For example, the fields of the `ls -l` command are always in the same character positions. 
